...
- HUMAN (R5P1 proof, do before re-recording): with a real camera, open npm run studio and confirm the capture chip reads >= 28 fps steady, then record ONE real take and check its measured fps in review (this is the Phase 1 "prove it with a real take" gate). Also glance at the D HUD RATES block in-game for hand/pose/render Hz and photon-to-emit with yourself in frame.
